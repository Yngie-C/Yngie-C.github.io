...
Skip-gram에서 경사 상승법을 통해 학습이 진행되는 과정을 살펴보겠습니다. 수식의 단순화를 위하여 $w_{t+j} \rightarrow o, w_t \rightarrow c$(outside, center)로 나타내면 주변 단어의 확률을 아래와 같이 나타낼 수 있습니다
$$
...
위 확률을 최대화하는 것이 목적입니다. $\text{argmax}$ 를 사용하면 식을 아래와 같이 나타낼 수 있습니다.


$$
\hat{v_c} = \text{argmax}_{v_c} p(o|c) = \text{argmax}_{v_c} \log p(o|c)
$$
...
네거티브 샘플링을 사용하지 않은 Skip-gram의 목적 함수는 
